...
namespace CS_Caixa.Controls
{
    public class ClassAto
    {
        ...
        public List<Ato> ListarAtoPorProtocoloRgi(int? protocolo)
        {
            try
            {
        ...
        public List<ItensAtoRgi> ListarItensAtoIdAtoRgi(int? IdAto)
        {
            try
            {
        ...
        public List<ItensCustasNota> CarregaItensCustasNotas(int idAtoNotas)
        {
            return contexto.ItensCustasNotas.Where(p => p.Id_Ato == idAtoNotas).ToList();
        }
        ...
        public string ExcluirAto(int id, string atribuicao)
        {
            ...
            try
            {
                if (atribuicao == "NOTAS")
                {
                    ...
                    var recibo = contexto.ReciboNotas.Where(p => p.AtoId == id).ToList();

                    foreach (var item in recibo)
                    {

                        var parte = contexto.Partes.Where(p => p.ParteId == item.ApresentanteId).ToList();
                        foreach (var item2 in parte)
                        {
                            contexto.Partes.Remove(item2);
                            contexto.SaveChanges();
                        }

                        contexto.ReciboNotas.Remove(item);
                        contexto.SaveChanges();
                    }
